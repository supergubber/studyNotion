              ...
              <li key={index}>
                {link.title === 'Catalog' ? (
                  <div className='relative flex flex-row items-center gap-2 cursor-pointer group'>
                    ...
                    <div className='invisible absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[30%] z-10 flex flex-col rounded-md bg-richblack-5 py-2 text-richblack-900 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100 lg:w-[200px]'>
                      ...
                      {subLinks.length ? (
                        subLinks.map((subLink, index) => (
                          <Link
                            to={`${subLink.links}`}
                            key={index}
                            className='z-10 px-4 py-2 hover:bg-richblack-50'
                          >
                            <p>{subLink.title}</p>
                          </Link>
                        ))
